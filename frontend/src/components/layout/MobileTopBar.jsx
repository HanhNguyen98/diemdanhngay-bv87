...
const MobileTopBar = memo(function MobileTopBar({ user, onNavChange, onMenuOpen, actions }) {
    ...
    <header className="lg:hidden sticky top-0 z-30 shrink-0 border-b border-line bg-surface-page/95 backdrop-blur-sm">
      <div className="flex items-center gap-2 px-[clamp(0.75rem,3vw,1.25rem)] py-2 min-h-[3rem]">
        <button
          type="button"
          onClick={onMenuOpen}
          className="w-9 h-9 rounded-lg border border-gray-200 bg-white flex items-center justify-center text-navy hover:bg-neutral transition-colors shrink-0"
          aria-label="Mở menu"
        >
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-4xs text-content-muted truncate">{UI.appSubtitleHead}</p>
          </div>
        </button>
